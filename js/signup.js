define(["jquery","input_ver"],function($,IV){
  ...
  function signup_input_verification(){
    let isYes = [false,false,false,false];//按钮是否可用
    //用户名的验证
    $("input.username").blur(function(){
      let value = $("input.username").val();
      let span_node = $("input.username").nextAll("span");
      let input_parent = $("input.username").parents('div.input');
      let result = null;
      if(span_node.length == 0){console.log("用户名提示信息节点获取错误");}
    ...
    $("input.email").blur(function(){
      ...
      let span_node = $("input.email").nextAll("span");
      let input_parent = $("input.email").parents('div.input');
      let result = null;
      if(span_node.length == 0){console.log("邮箱提示信息节点获取错误");}
    ...
    $("input.tel").blur(function(){
      let value = $("input.tel").val();
      let span_node = $("input.tel").nextAll("span");
      let input_parent = $("input.tel").parents('div.input');
      let result = null;
      if(span_node.length == 0){console.log("手机号提示信息节点获取错误");}
    ...
    $("input.password").blur(function(){
      ...
      let span_node = $("input.password").nextAll("span");
      let input_parent = $("input.password").parents('div.input');
      let result = null;
      if(span_node.length == 0){console.log("密码提示信息节点获取错误");}
  ...
  function signup_verification(){
    $(".submit").click(function (e) { 
      ...
      let value_email = $("input.email").val();
      let value_tel = $("input.tel").val();
      ...
      if(!value_email && !value_tel){
        $(".btn.btn_primary").addClass("disabled");
        $(".t_required").addClass("input_error")
                        .parents('div.input')
                        .addClass("invalid");
        $(".e_required").addClass("input_error")
                        .parents('div.input')
                        .addClass("invalid");
      }
      if(!value_pwd){
      ...
      if(value_pwd && value_username && (value_email || value_tel)){
        ...
        let createTime = "" + 
        time.getFullYear() + "-" + 
        double_num(time.getMonth()+1) + "-" + 
        double_num(time.getDate()) + " " + 
        double_num(time.getHours()) + ":" + 
        double_num(time.getMinutes()) + ":" + 
